...
5. 脚本会在上传前对每个文件计算 SHA-256，并按当前登录账号批量查询已存在素材；内容重复的文件会跳过上传，并报告已存在素材名称和数据库 ID。同一批次内内容相同的文件也只上传一次。去重检查失败时脚本会停止上传，避免误产生重复素材。
6. 等脚本输出每个文件的成功、跳过或失败结果后再向用户汇报。成功项必须包含素材名称和数据库 ID；跳过项必须说明对应的已存在素材；失败项说明文件名和服务端错误，不把失败说成已上传。
